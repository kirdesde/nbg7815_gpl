...
#define UINT32(x)  (sizeof(u32_t)==4 ? (u32_t)(x):((x)&0xffffffff))
...
#if WORDS_BIGENDIAN == 0
#define SET_VALUE(byte_mode,addr,value) ((byte_mode) ? (*(unsigned char *)(addr)=(value)):(*(u32_t *)(addr)=((u32_t)(value))))
#else
...
u32_t rar_crc(u32_t start_crc, void *addr, u32_t size)
{
	unsigned char *data;
	int i;
...
{
	rarvm_data->mem = (u8_t *) malloc(RARVM_MEMSIZE + 4);
	init_crc();
	if (!rarvm_data->mem)
	{
		return FALSE;
...
void rarvm_set_memory(rarvm_data_t *rarvm_data, unsigned int pos, u8_t *data, unsigned int data_size)
{
...
static void execute_standard_filter(rarvm_data_t *rarvm_data, rarvm_standard_filters_t filter_type)
{
	...
	case VMSF_E8E9:
		...
		rar_dbgmsg("data size %d\n", data_size);
		file_offset = rarvm_data->R[6];
		rar_dbgmsg("file off %d\n", file_offset);
                              ...
                              struct rarvm_prepared_command *prepared_code, int code_size)
{
	...
	while (1)
	{
		...
		rar_dbgmsg("op(%d) op_code: %d, op1=%u, op2=%u\n", 25000000 - max_ops,
		           cmd->op_code, (unsigned int)op1, (unsigned int)op2);
		switch (cmd->op_code)
		{
		case VM_MOV:
...
int rarvm_execute(rarvm_data_t *rarvm_data, struct rarvm_prepared_program *prg)
{
	...
	if (data_size != 0)
	{
		...
		if (!prg->global_data)
		{
			rar_dbgmsg("unrar: rarvm_execute: realloc failed for prg->global_data\n");
			return FALSE;
		}
                  ...
                  int code_size, struct rarvm_prepared_program *prg)
{
	...
	if (xor_sum == code[0])
	{
		...
		if (data_flag & 0x8000)
		{
			...
			if (!prg->static_data)
			{
				rar_dbgmsg("unrar: rarvm_prepare: malloc failed for prg->static_data\n");
				return FALSE;
			}
#ifdef LOOP_DEBUG
			loop_idx = 835;
			...
			for (i = 0 ; rarvm_input->in_addr < code_size && i < data_size ; i++)
			{
				...
				if (!prg->static_data)
				{
					rar_dbgmsg("unrar: rarvm_prepare: realloc failed for prg->static_data\n");
#ifdef LOOP_DEBUG
					loop_idx = 836;
#endif
					return FALSE;
