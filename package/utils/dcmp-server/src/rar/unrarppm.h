...
typedef struct sub_allocator_tag
{
	...
	i16_t indx2units[N_INDEXES], units2indx[128], glue_count;
	u8_t *heap_start, *lo_unit, *hi_unit;
	struct rar_node free_list[N_INDEXES];

	u8_t *ptext, *units_start, *heap_end, *fake_units_start;
} sub_allocator_t;
...
typedef struct ppm_data_tag
{
	...
	u8_t char_mask[256], ns2indx[256], ns2bsindx[256], hb2flag[256];
	u8_t esc_count, prev_success, hi_bits_flag;
	struct see2_context_tag see2cont[25][16], dummy_sse2cont;
	u16_t bin_summ[128][64];
} ppm_data_t;

int ppm_decode_init(ppm_data_t *ppm_data, struct unpack_data_tag *unpack_data, int *EscChar);
int ppm_decode_char(ppm_data_t *ppm_data, struct unpack_data_tag *unpack_data);
