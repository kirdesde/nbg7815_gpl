                                      ...
                                      int old_indx, int new_indx)
{
	...
	p = ((u8_t *) pv) + sub_allocator_u2b(sub_alloc->indx2units[new_indx]);
...
static int sub_allocator_start_sub_allocator(unpack_data_t *unpack_data, sub_allocator_t *sub_alloc, int sa_size)
{
	...
	rar_dbgmsg("PPM sub allocator %d\n", alloc_size);
	if (alloc_size > RAR_MAX_PPM_MEM)
	{
		//printf("allocate size %d, %d, %d\n", t, FIXED_UNIT_SIZE*UNIT_SIZE, alloc_size);
		//unpack_data->rar_error |= RAR_REACH_MAX_PPM_MEM_ERROR;
		//return FALSE;
		alloc_size = RAR_MAX_PPM_MEM;
		t = 31457268;
	}
	if ((sub_alloc->heap_start = (u8_t *) malloc(alloc_size)) == NULL)
	{
		rar_dbgmsg("sub_alloc start failed\n");
		unpack_data->rar_error |= RAR_MALLOC_ERROR;
		return FALSE;
	}
...
static void range_coder_init_decoder(range_coder_t *coder,
                                     unpack_data_t *unpack_data)
{
	...
	for (i = 0; i < 4 ; i++)
	{
		coder->code = (coder->code << 8) | rar_get_char(unpack_data);
	}
}

static int coder_get_current_count(range_coder_t *coder)
{
	if ((coder->scale != 0) && ( (coder->range / coder->scale) != 0))
	{
...
#define ARI_DEC_NORMALISE(unpack_data, code, low, range)					\
{												\
	while ((low^(low+range)) < TOP || (range < BOT && ((range=-low&(BOT-1)),1))) {		\
		code = (code << 8) | rar_get_char(unpack_data);				\
		range <<= 8;									\
		low <<= 8;									\
	}											\
}
...
{
	int i, k, m;
	static const u16_t init_bin_esc[] =
	{
	...
	{
		rar_errmsg("unrar: restart_model_rare: sub_allocator_alloc_context failed\n");
		return FALSE;
	}
	...
	{
		rar_errmsg("unrar: restart_model_rare: sub_allocator_alloc_units failed\n");
		return FALSE;
	}
...
NO_LOOP:
	...
	up_state.symbol = *(u8_t *) up_branch;
	up_state.successor = (struct ppm_context *) (((u8_t *) up_branch) + 1);
	if (pc->num_stats != 1)
	{
		if ((u8_t *) pc <= ppm_data->sub_alloc.ptext)
		{
			return NULL;
		}
...
static int update_model(ppm_data_t *ppm_data)
{
	...
	*ppm_data->sub_alloc.ptext++ = fs.symbol;
	...
	if (fs.successor)
	{
		if ((u8_t *)fs.successor <= ppm_data->sub_alloc.ptext &&
...
static const u8_t ExpEscape[16] = { 25, 14, 9, 7, 5, 5, 4, 4, 4, 3, 3, 3, 2, 2, 2, 2 };
...
static void ppm_decode_bin_symbol(ppm_data_t *ppm_data, struct ppm_context *context)
{
	...
	if (coder_get_current_shift_count(&ppm_data->coder, TOT_BITS) < *bs)
	{
		...
		*bs = (u16_t) (*bs + INTERVAL - GET_MEAN(*bs, PERIOD_BITS, 2));
	...
	else
	{
		...
		*bs = (u16_t) (*bs - GET_MEAN(*bs, PERIOD_BITS, 2));
...
int ppm_decode_init(ppm_data_t *ppm_data, unpack_data_t *unpack_data, int *EscChar)
{
	int max_order, Reset, MaxMB = 0;

	max_order = rar_get_char(unpack_data);
	rar_dbgmsg("ppm_decode_init max_order=%d\n", max_order);
	Reset = (max_order & 0x20) ? 1 : 0;
	rar_dbgmsg("ppm_decode_init Reset=%d\n", Reset);
	if (Reset)
	{
		MaxMB = rar_get_char(unpack_data);
	...
	if (max_order & 0x40)
	{
		*EscChar = rar_get_char(unpack_data);
		rar_dbgmsg("ppm_decode_init EscChar=%d\n", *EscChar);
	}
	range_coder_init_decoder(&ppm_data->coder, unpack_data);
	if (Reset)
	{
		...
		if (!sub_allocator_start_sub_allocator(unpack_data, &ppm_data->sub_alloc, MaxMB + 1))
		{
			sub_allocator_stop_sub_allocator(&ppm_data->sub_alloc);
			return FALSE;
		}
...
int ppm_decode_char(ppm_data_t *ppm_data, unpack_data_t *unpack_data)
{
	int symbol;

	if ((u8_t *) ppm_data->min_context <= ppm_data->sub_alloc.ptext ||
	        (u8_t *)ppm_data->min_context > ppm_data->sub_alloc.heap_end)
	{
		return -1;
	}
	if (ppm_data->min_context->num_stats != 1)
	{
		if ((u8_t *)(ppm_data->min_context->con_ut.u.stats) <= (ppm_data->sub_alloc.ptext) ||
		        (u8_t *)(ppm_data->min_context->con_ut.u.stats) > (ppm_data->sub_alloc.heap_end))
		{
			return -1;
		}
	...
	{
		ARI_DEC_NORMALISE(unpack_data, ppm_data->coder.code,
	...
	if (!ppm_data->order_fall && (u8_t *) ppm_data->found_state->successor > ppm_data->sub_alloc.ptext)
	{
	...
	ARI_DEC_NORMALISE(unpack_data, ppm_data->coder.code, ppm_data->coder.low,
