			...
			struct nf_hook_ops **elemp)
{
	...
	list_for_each_entry_continue_rcu((*elemp), head, list) {
		...
		{
			int zyinetpkt_enable = zld_export_cb.zyinetpkt_enable;
			if(unlikely(zyinetpkt_enable)) {
				typeof(zld_export_cb.zyinetpkt_skb_dump) dumpfn;
				dumpfn = rcu_dereference(zld_export_cb.zyinetpkt_skb_dump);
				if ( likely(dumpfn != NULL) ) {
					dumpfn(UINT_MAX, *elemp, skb, state->in, state->out, jiffies);
				}
			}
#endif
...
repeat:
		verdict = (*elemp)->hook((*elemp)->priv, skb, state);
			...
			if(unlikely(zyinetpkt_enable)) {
				...
				dumpfn = rcu_dereference(zld_export_cb.zyinetpkt_skb_dump);
				if ( likely(dumpfn != NULL) ) {
					dumpfn(verdict, *elemp, skb, state->in, state->out, jiffies);
				}
			}
		}
#endif
