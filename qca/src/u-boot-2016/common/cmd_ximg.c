...
int zyxel_imxtract_fit(const unsigned char *buf, const char *name, unsigned int *addr, unsigned int *size)
{
	...
	format = genimg_get_format(buf);
	if (format != IMAGE_FORMAT_FIT) {
		puts("Not FIT image format\n");
		return 1;
	}
	...
	noffset = fit_image_get_node(fit_hdr, name);
	if (noffset < 0 ) {
		printf("Can't find '%s' FIT subimage\n", name);
		return 1;
	}
	...
	if (!fit_image_check_comp(fit_hdr, noffset, IH_COMP_NONE)) {
		puts("Compressed subimage, wrong\n");
		return 1;
	}

	if (fit_image_get_data(fit_hdr, noffset, &fit_data, size)) {
		puts("Could not find subimage data\n");
		return 1;
	}

	*addr = (unsigned int) fit_data;
