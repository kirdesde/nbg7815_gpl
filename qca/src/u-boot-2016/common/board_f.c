...
#if defined(CONFIG_ZYXEL_RESET_BTN)
static unsigned int zyxel_reset_gpio = 9999;
int zyxel_get_reset_button(void)
{
	if (zyxel_reset_gpio != 9999)
		return (gpio_get_value(zyxel_reset_gpio) == GPIO_OUT_LOW)?1:0;
	else
		return 0;
}
#else
int zyxel_get_reset_button(void) { return 0; }
#endif
...
int misc_init_f(void)
{
	...
	node = fdt_path_offset(gd->fdt_blob, "/buttons");
	if (node > 0) {
		qca_gpio_init(node);
	}
	#if defined(CONFIG_ZYXEL_RESET_BTN)
	if ( node > 0 ) {
		...
		for (offset = fdt_first_subnode(gd->fdt_blob, offset); offset > 0;
			...
			if (!strcmp("reset", fdt_get_name(gd->fdt_blob, offset, NULL))) {
				zyxel_reset_gpio = fdtdec_get_uint(gd->fdt_blob, offset, "gpio", 0);
			}
		}
	}
	#endif
