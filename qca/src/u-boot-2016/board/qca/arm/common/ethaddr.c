...
int get_eth_mac_address(uchar *enetaddr, uint no_of_macs)
{
	...
	char mmc_blks[512];
#endif
#ifdef CONFIG_ZYXEL_ETHADDR
	if (getenv("ethaddr")) {
		char mac[64];
		char *p;
		int i;
		memset(mac, 0, 64);
		snprintf(mac, 64, getenv("ethaddr"));
		p = mac;
		for (i = 0 ; i < 6; i++) {
			enetaddr[i] = simple_strtoul(p, &p, 16);
			if ((*p) == ':') p++;
		}
		return 0;
	}
...
void set_ethmac_addr(void)
{
	...
	memset(mac, 0, 64);
	if (!getenv(ethaddr)) {
		puts("No ethaddr in environment, using default value ...\n\n");
	...
	case '8':
		...
		break;
	default:
		puts("Not Zyxel ethaddr environment ...\n");
		break;
	}
	if (zyxel) {
		//setenv(CONFIG_ZYXEL_ETHADDR_WAN, mac);
		return;
	}
#endif
