...
static void (*zld_multiboot_handler)(u8 *, unsigned, unsigned, unsigned) = NULL;
void net_set_multiboot_handler(void (*handler)(u8 *, unsigned, unsigned, unsigned))
{
...
#if defined(CONFIG_ZYXEL)
		//printf("det_ip=0x%08X(0x%08X)\n", dst_ip.s_addr, htonl(dst_ip.s_addr));
		if (dst_ip.s_addr == htonl(0xe1000000) && zld_multiboot_handler!= NULL) {
			 (*zld_multiboot_handler)((u8 *)ip + IP_UDP_HDR_SIZE,
					 ntohs(ip->udp_dst),
					 ntohs(ip->udp_src),
...
#if defined(CONFIG_ZYXEL)
u32 getNetBootFileXferSize(void)
{
